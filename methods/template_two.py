...
class MetaTemplate(nn.Module):
    ...
    def parse_feature(self, x, is_feature):
        ...
        if is_feature:
            ...
            '''
            #add cross-attention
            z_all = x.view(self.n_way, self.n_support + self.n_query, x.shape[1], x.shape[2], x.shape[3])
        z_support = z_all[:, :self.n_support]
        z_query = z_all[:, self.n_support:]

        #first caculate sam,then bdc
        z_support, z_query = self.cam(z_support, z_query)#5,16,1,640,10,10
        #z_support_norm = F.normalize((z_support, p=2, dim = 3, eps=1e-12))
        z_support =z_support.mean(1)
        z_query = z_query.mean(2)
        z_support = z_support.reshape(self.n_way*self.n_support, x.shape[1], x.shape[2], x.shape[3])
        z_query = z_query.reshape(self.n_way*(self.n_query), x.shape[1], x.shape[2], x.shape[3])
        z_support_bdc = self.feature_forward(z_support)
        z_query_bdc = self.feature_forward(z_query)
            '''
            #add qkv attention
        ...
        z_support_cross, z_query_cross = self.cross_attention(z_support_self, z_query_self)#5,5,16,640,10,10,  5,16,5,,640,10,10

        z_support_bdc = self.feature_forward(z_support_cross)
