...
class BaselineTrain(nn.Module):
    def __init__(self, params, model_func, num_class):
        ...
        if params.method in ['stl_deepbdc', 'meta_deepbdc', 'protonet', 'good_embed']:
            self.reduce_feat_dim = int(reduce_dim * (reduce_dim+1) / 2)#poooling
...
class MetaTemplate(nn.Module):
    ...
    def parse_feature(self, x, is_feature):
        ...
        if is_feature:
            ...
            x = x.contiguous().view(self.n_way * (self.n_support + self.n_query), *x.size()[-3:])
        ...
        #z_support_self_1 = z_support_self.reshape(5, -1, *z_support_self.shape[2:])
        z_support_self = z_support_self.reshape(5, -1, *z_support_self.shape[2:]).mean(1)
        z_query_self = z_query_self.reshape(-1, *z_query_self.shape[2:])
        z_support_cross, z_query_cross = self.cross_attention(z_support_self, z_query_self, self.omga)#5,5,16,640,10,10,  5,16,5,,640,10,10
