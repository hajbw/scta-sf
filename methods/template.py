...
#from kstest import visualize_convolutional_features, gaussianity_tests_convolutional, print_results_summary, \
#    save_results_to_json, \
#    generate_detailed_report, safe_tensor_to_numpy
...
class BaselineTrain(nn.Module):
    def __init__(self, params, model_func, num_class):
        ...
        if params.method in ['stl_deepbdc', 'meta_deepbdc', 'protonet', 'good_embed']:
            self.reduce_feat_dim = 205120#poooling
...
class MetaTemplate(nn.Module):
    ...
    def parse_feature(self, x, is_feature):
        ...
        if is_feature:
            ...
            '''
            conv_features=torch.pow(torch.abs(x), 0.3)
            
            # 1. 可视化特征
            print("\n生成特征可视化...")
            visualize_convolutional_features(conv_features, sample_channels=4)

            # 2. 执行高斯性检验
            print("\n执行高斯性检验...")
            results = gaussianity_tests_convolutional(
                features=conv_features,
                sample_channels=50,  # 测试50个通道
                sample_spatial=20,  # 测试20个空间位置
                significance_level=0.05,
                test_name="ResNet_Features"
            )
...
class MetaTemplate2(nn.Module):
    def __init__(self, params, model_func, n_way, n_support, change_way=True):
        super(MetaTemplate2, self).__init__()
        self.n_way = n_way
        self.n_support = n_support
        self.n_query = params.n_query  # (change depends on input)
        self.feature = model_func()
    ...
    def parse_feature(self, x, is_feature):
        ...
        # z_support_cross, z_query_cross = self.cross_attention(z_support_self, z_query_self)#5,5,16,640,10,10,  5,16,5,,640,10,10
        z_support_cross, z_query_cross = z_support_self, z_query_self
