...
from .template_three import MetaTemplate
...
class MetaDeepBDC(MetaTemplate):#父类是metatemplate，对三个抽象方法重写
    ...
    def __init__(self, params, model_func, n_way, n_support):
        ...
        self.metric_func = self.cosine_distance_score if params.distance == 'cosine' else self.euclidean_distance_score if params.distance == 'euclidean' else self.inner_product_score
    ...
    def set_forward(self, x, is_feature=False):#前向传播，返回距离
        ...
        z_proto = z_support.contiguous().view(self.n_way, self.n_query, -1)
        ...
        z_support = z_support.reshape(self.n_way, 1, -1)#5,16,1,2064
    ...
    def euclidean_distance_score(self, x, y):#欧氏距离
        return -torch.pow(x - y, 2).sum(2)
    
    def inner_product_score(self, x, y):#内积
        return (x * y).sum(2)
    
    def cosine_distance_score(self, x, y):#余弦距离
