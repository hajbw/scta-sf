...
class CAM(nn.Module):
    ...
    def forward(self, f1, f2):
        ...
        f1 = f1.view(b, n1, c, -1) #5,5,640,100
        f2 = f2.view(b, n2, c, -1)#5,16,640,100

        f1_norm = F.normalize(f1, p=2, dim=2, eps=1e-12)
        f2_norm = F.normalize(f2, p=2, dim=2, eps=1e-12)
        
        f1_norm = f1_norm.transpose(2, 3).unsqueeze(2) #5,5,1,100,640
        f2_norm = f2_norm.unsqueeze(1)#5,1,16,640,100

        a1 = torch.matmul(f1_norm, f2_norm) #5,5,16,100,100
        a2 = a1.transpose(3, 4) 
