...
#from kstest import visualize_distributions, visualize_q_q_plot, gaussianity_tests, analyze_results

# from .cam import CAM
# from .attention_three import SelfAttention, CrossAttention
# from .attention_tiered import SelfAttention, CrossAttention
class BaselineTrain(nn.Module):
    def __init__(self, params, model_func, num_class):
        ...
        if params.method in ['stl_deepbdc', 'meta_deepbdc', 'protonet', 'good_embed']:
            self.reduce_feat_dim = 2064  # poooling
            self.classifier = nn.Linear(self.reduce_feat_dim, num_class)
...
class MetaTemplate(nn.Module):
    def __init__(self, params, model_func, n_way, n_support, change_way=True):
        ...
        if self.params.dataset == 'cub':
            from .attention_cub import SelfAttention, CrossAttention
            self.self_attention = SelfAttention(512)
            self.cross_attention = CrossAttention(512)
        else:
            if self.params.dataset == 'mini_imagenet':
                from .attention_mini import SelfAttention, CrossAttention
            elif self.params.dataset == 'tiered_imagenet':
                from .attention_tiered import SelfAttention, CrossAttention
            elif self.params.dataset == 'fc100':
                from .attention_fc import SelfAttention, CrossAttention
            elif self.params.dataset == 'cifar':
                from .attention_fc import SelfAttention, CrossAttention
            else:
                from .attention_mini import SelfAttention, CrossAttention
            self.self_attention = SelfAttention(640)
            self.cross_attention = CrossAttention(640)
    ...
    def parse_feature(self, x, is_feature,bypass_cross_attention=False,bypass_self_attention=False):
        x = Variable(x.cuda())
        if is_feature:
            z_all = x
        else:
            ...
            if bypass_self_attention:
                x_self = x
            else:
                x_self = self.self_attention(x)
            # add qkv attention
            z_all = x_self.view(self.n_way, self.n_support + self.n_query, x.shape[1], x.shape[2], x.shape[3])

        z_support_self = z_all[:, :self.n_support]
        z_query_self = z_all[:, self.n_support:]

        # first caculate attention,then bdc
        z_support_self = z_support_self.reshape(-1, *z_support_self.shape[2:])
        z_query_self = z_query_self.reshape(-1, *z_query_self.shape[2:])
        if bypass_cross_attention:
            z_support_bdc = self.feature_forward(z_support_self)
            z_query_bdc = self.feature_forward(z_query_self)
        else:
            z_support_cross, z_query_cross = self.cross_attention(z_support_self, z_query_self)  # 5,5,16,640,10,10,
            z_support_bdc = self.feature_forward(z_support_self)
            z_query_bdc = self.feature_forward(z_query_cross)

        return z_support_bdc, z_query_bdc
